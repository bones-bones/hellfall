...
function transformCard(card) {
  ...
  const out = {
    ...
    'Supertype(s)': arr4(
      Array.isArray(f0 && f0.supertypes)
        ? f0.supertypes.join(' ')
  ...
  const tokenParts = (card.all_parts || []).filter(p => p && p.component === 'token');
...
function transformHellscubeDatabase(sourceJson) {
  const parsed = typeof sourceJson === 'string' ? JSON.parse(sourceJson) : sourceJson;
