...
const loadExistingData = () => {
  ...
  const existingTokens = new HCIDMap(
    dataToCards(tokensContent?.data ?? [], 'oracle_id', '', 'parts')
  );
  ...
  // #jank
  const existingLands = new HCIDMap(
    dataToCards(landsContent?.data ?? [], 'oracle_id', '', 'parts')
  );
