...
| `/api/cards/load`                | GET      | Full card catalog as `{ data: HCCard[] }`. In-memory cache (default 24h, `CATALOG_CACHE_TTL_MS`). On miss: GCS snapshot if `CATALOG_GCS_BUCKET` set, else bundled JSON — not Firestore. Refreshed after changeset accept (debounced export). `Cache-Control: public, 3 days`. |
...
2. **Environment variables** (create `.env` in `packages/server/`, or set in your host’s config)
   ...
   - **Catalog snapshot:** `/api/cards/load` reads bundled JSON or GCS — not Firestore on cache miss. Optional `CATALOG_GCS_BUCKET`: on changeset accept, debounced Firestore export uploads `catalog.json` + `catalog-manifest.json`. Cloud Run SA needs `storage.objectAdmin` (publish) and `objectViewer` (read). `CATALOG_PUBLISH_DEBOUNCE_MS` defaults to 30000; `CATALOG_CACHE_TTL_MS` defaults to 86400000 (24h).
