...
2. **Environment variables** (create `.env` in `packages/server/`, or set in your host’s config)
   ...
   - **Catalog snapshot:** `/api/cards/load` reads bundled JSON or GCS — not Firestore on cache miss. Default bucket **`hellscube-images`** (`CATALOG_GCS_BUCKET`): `POST /api/admin/catalog/sync` (or postcard ingest) uploads gzip-compressed `catalog.json` + `catalog-manifest.json`. Browser `fetch` uses `CARD_CATALOG_URL` (defaults to `https://storage.googleapis.com/hellscube-images/catalog.json`). Apply [misc/cors.json](../../misc/cors.json) to the bucket.
