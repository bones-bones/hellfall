...
| `/api/cards/postcard`             | POST     | Mork ingest: upsert sparse accepted card to Firestore (`{ name, image \| imageBase64, imageMimeType?, creators, set, hcid?, kind? }`). When `imageBase64` is sent, hellfall uploads to `IMAGE_GCS_CARD_IMAGE_BUCKET` (object suffix from magic bytes / `imageMimeType`, not always `.png`) and stores the public GCS URL. `Authorization: Bearer <MORK_POSTCARD_API_KEY>`. Returns `{ docId, id, wasCreate, imageUrl, previous? }`. Triggers debounced catalog publish. |
