...
| `/api/cards/postcard`            | POST     | Mork ingest: upsert sparse accepted card to Firestore (`{ name, image, creators, set, hcid?, kind? }`). `Authorization: Bearer <MORK_POSTCARD_API_KEY>`. Returns `{ docId, cardId, wasCreate, previous? }`. Triggers debounced catalog publish.                                                                                                                 |
