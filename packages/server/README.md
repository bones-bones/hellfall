...
| `/api/cards/search?q=query`       | GET      | Search results. Optional `format`: `json`, `xml`, `cockatrice`, `draftmancer`, `tabletopsimulator`.                                                                                                                                                                                                                                                                               |
| `/api/cards/random?q=query`       | GET      | Gets a random card from a search. Query optional.                                                                                                                                                                                                                                                                                                                                 |
| `/api/cards/postcard`             | POST     | Mork ingest: upsert sparse accepted card to Firestore (`{ name, image \| imageBase64, creators, set, hcid?, kind? }`). When `imageBase64` is sent, hellfall uploads to `IMAGE_GCS_CARD_IMAGE_BUCKET` and stores the public GCS URL. `Authorization: Bearer <MORK_POSTCARD_API_KEY>`. Returns `{ docId, id, wasCreate, imageUrl, previous? }`. Triggers debounced catalog publish. |
| `/api/cards/postcard/rollback`    | POST     | Undo a postcard write when mork sheet update fails (`{ docId, wasCreate, previous? }`). Same Bearer auth.                                                                                                                                                                                                                                                                         |
...
| `/api/mork`                       | GET      | Handles a mork command. See [mork's code](https://github.com/bones-bones/mork/blob/main/cogs/HellscubeDatabase.py) for the other side of the requests.                                                                                                                                                                                                                            |
| `/api/admin/catalog/sync`         | POST     | Accept catalog republish job → **202** `{ jobId, runPath, statusPath }`. Does not publish yet.                                                                                                                                                                                                                                                                                    |
| `/api/admin/catalog/sync/:id`     | GET      | Job status (`queued` / `running` / `completed` / `failed`).                                                                                                                                                                                                                                                                                                                       |
| `/api/admin/catalog/sync/:id/run` | POST     | Run the publish while the request stays open (Firestore → cache + GCS). Returns `{ ok, cardCount, version, … }`.                                                                                                                                                                                                                                                                  |
...
2. **Environment variables** (create `.env` in `packages/server/`, or set in your host’s config)
   ...
   - **Catalog snapshot:** `/api/cards/load` reads bundled JSON or GCS — not Firestore on cache miss. Default bucket **`hellscube-images`** (`CATALOG_GCS_BUCKET`): `POST /api/admin/catalog/sync` accepts a job (**202**); `POST /api/admin/catalog/sync/:jobId/run` uploads gzip-compressed `catalog.json` + `catalog-manifest.json`. Postcard ingest still debounces an in-process publish. Browser `fetch` uses `CARD_CATALOG_URL` (defaults to `https://storage.googleapis.com/hellscube-images/catalog.json`). Apply [misc/cors.json](../../misc/cors.json) to the bucket.
   - **Image hosting (mork postcard):** `IMAGE_GCS_CARD_IMAGE_BUCKET` (default **`hellscube-images`**). Cloud Run SA needs `storage.objectAdmin`; objects must be publicly readable.
   - **Mork postcard auth:** `MORK_POSTCARD_API_KEY` — Bearer token for `POST /api/cards/postcard` and `/rollback`.
