...
import { getPreference, preferType } from './preferenceHandling';
import { extraSetList, getChildSets, getDirectChildSets } from '../setDateHandling';
...
export class LightCardMap {
   ...
   */
  getPreferredByOracleId(oracle_id: string, prefer?: preferType): HCCard.Any | undefined {
   ...
   * @param prefer the version of the card to prefer, if any
   */
  getCardsByOracleIds(oracleList: stringIterable, prefer?: preferType): HCCard.Any[] {
    const cards: HCCard.Any[] = [];
   ...
   * @param prefer the version of the card to prefer, if any
   */
  getCardsByOracleIdsAsSubset(oracleList: stringIterable, prefer?: preferType): this {
    const subMap = new (this.constructor as any)() as this;
    for (const id of oracleList) {
      const prints = this.getAllPrints(id);
      if (prints.length) {
